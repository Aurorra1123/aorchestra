...
7. You can add thinking comments using # (bash standard comments will be ignored)

RESPONSE FORMAT:
```bash
# thinking: your reasoning here (optional)
single_command_here
```
...
```bash
# thinking: I need to create this file first
cat << 'EOF' > file.txt
content
EOF
```
...
```bash
# thinking: your reasoning (optional, using standard bash comments)
your_command_here
```
...
- You can add # thinking: comments for clarity
...
class MiniSWEAgent(BaseAgent):
    """Simplified MiniSWE agent for local execution."""

    name: str = "MiniSWEAgent"
    description: str = "Simplified mini-swe-agent with local support"
